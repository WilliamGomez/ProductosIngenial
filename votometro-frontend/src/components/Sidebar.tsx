...
export default function Sidebar() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        console.warn(
          "[Sidebar] product catalog unavailable; using fallback reports",
          error,
        );
      }
    })();
  ...
  const reportSource =
    userRole === "Admin" && productCatalog.length > 0
      ? productCatalog
      : (userProducts ?? []);

  const reportItems = reportSource
    .map((product) => {
      ...
      return {
        ...
        icon: resolveLucideIcon(product.icon, fallback?.icon || FileBarChart),
        order: product.display_order ?? fallback?.order ?? 100,
        visible:
          product.is_report_enabled !== false &&
          (userRole === "Admin" ||
            isProductActive(product as Parameters<typeof isProductActive>[0])),
      };
    })
    .filter((item) => item.visible)
    .sort((a, b) => a.order - b.order);

  const adminFallbackReportItems =
    userRole === "Admin" && reportItems.length === 0 ? fallbackReports : [];

  const items = [
    ...reportItems,
    ...adminFallbackReportItems,
    ...NAV_ITEMS.filter((item) => item.visible({ userRole, hasProduct })),
  ];
