...
const Votometro = () => {
  const REPORT_ID = "9db4c8ee-d117-4a2e-9a72-9284c6208fa0";
      ...
      try {
        ...
        if (reportRef.current) {
          try {
            ...
            console.error("[Votometro] Failed to update access token, re-embedding", error);
            reportRef.current = null;
            setEmbedKey((prev) => prev + 1);
          }
        }

        console.info(`[Votometro] Embed config refreshed (${reason})`);
      } catch (error) {
        console.error("[Votometro] Failed to refresh embed config", error);
      }
    },
  ...
  const resetPowerBiReport = useCallback(() => {
    try {
      ...
      console.warn("[Votometro] Failed to reset Power BI report", error);
    } finally {
      reportRef.current = null;
